<template>
    ...
    >
        <div class="group">
            <CheckCircleIcon v-if="task.completed_at"
             @mouseenter="toggleTooltip()"
             @mouseleave="toggleTooltip()"
            class="h-5 float-right text-green-500"/>
            <div class="text-left rounded py-2 px-4 z-10 w-100 absolute shadow-lg shadow-black right-3 bg-[#082F49] text-white invisible group-hover:visible">
                <span class="underline">Completed At</span><br>{{ task.completed_at }}
            </div>
        </div>
        {{ task.name }}<br>
